...
class OctoPiPanel():
    ...
    def __init__(self, caption="OctoPiPanel"):
        ...
        """
        self.done = False
        self.color_bg = pygame.Color(50, 150, 70)

        # Button settings
        self.leftPadding = 5
        self.buttonSpace = 100 if (self.win_width > 320) else 5
        self.buttonWidth = 100 #(self.win_width - self.leftPadding * 2 - self.buttonSpace * 2) / 3
	...
	#self.screen = pygame.display.set_mode(modes[0], FULLSCREEN, 16)
        ...
        self.fntTextSmall = pygame.font.Font(os.path.join(self.scriptDirectory, "Cyberbit.ttf"), 14)
        ...
        # Home X/Y/Z buttons
        self.btnHomeXY        = pygbutton.PygButton((  self.colOne,   self.rowOne, self.buttonWidth, self.buttonHeight), "HOME X/Y", (45,95,180))
        self.btnHomeZ         = pygbutton.PygButton((  self.colOne,  self.rowTwo, self.buttonWidth, self.buttonHeight), "HOME Z", (45,95,180))
        self.btnZUp           = pygbutton.PygButton((  self.colThree,  self.rowOne, self.buttonWidth, self.buttonHeight), "Z +25", (45,95,180))

        # Heat buttons
        self.btnHeatBed       = pygbutton.PygButton((  self.colTwo,  self.rowOne, self.buttonWidth, self.buttonHeight), "HeatBed", (255,0,0))
        self.btnHeatHotEnd    = pygbutton.PygButton((  self.colTwo,  self.rowTwo, self.buttonWidth, self.buttonHeight), "HeatHotEnd", (255,0,0))

        # Start, stop and pause buttons
        self.btnStartPrint    = pygbutton.PygButton((  self.colOne, self.rowOne, self.buttonWidth, self.buttonHeight), "Start", (80,140,60))
        self.btnAbortPrint    = pygbutton.PygButton((  self.colOne, self.rowOne, self.buttonWidth, self.buttonHeight), "Abort", (80, 0, 0))
        self.btnPausePrint    = pygbutton.PygButton((  self.colOne, self.rowTwo, self.buttonWidth, self.buttonHeight), "Pause", (80,111,155))

        # Shutdown and reboot buttons
        self.btnReboot        = pygbutton.PygButton((  self.colThree,   self.rowTwo, self.buttonWidth, self.buttonHeight), "REBOOT", (52,111,155));
        self.btnShutdown      = pygbutton.PygButton((  self.colThree,  self.rowTwo, self.buttonWidth, self.buttonHeight), "SHUTDOWN", (52,111,155));
    ...
    def update(self):
        ...
        if self.Paused:
            self.btnPausePrint.caption = "Restart"
        else:
        ...
        if self.HotHotEnd:
            self.btnHeatHotEnd.caption = "Extruder Off"
        else:
            self.btnHeatHotEnd.caption = "Extruder Heat"

        if self.HotBed:
            self.btnHeatBed.caption = "BED Off"
        else:
            self.btnHeatBed.caption = "BED Heat"
    ...
    def draw(self):
        ...
        lblHotEndTemp = self.fntText.render(u'Tete: {0}\N{DEGREE SIGN}C ({1}\N{DEGREE SIGN}C)'.format(self.HotEndTemp, self.HotEndTempTarget), 1, (220, 0, 0))
        #self.screen.blit(lblHotEndTemp, (self.leftPadding + self.buttonWidth + self.buttonSpace, 60))
        lblBedTemp = self.fntText.render(u'Lit: {0}\N{DEGREE SIGN}C ({1}\N{DEGREE SIGN}C)'.format(self.BedTemp, self.BedTempTarget), 1, (66, 100, 255))
        ...
        lblPrintTimeLeft = self.fntText.render("Temps restant : {0}".format(datetime.timedelta(seconds = self.PrintTimeLeft)), 1, (200, 200, 200))
    ...
    def _heat_hotend(self):
        ...
        if self.HotHotEnd:
            ...
            data = { "command": "target", "targets": { "tool0": 205 } }
